...
DATA_PATH="/home/users0/jagfelga/slu-work/NLG/RNNLG-Laptop/data_word_based_delex/"
EXPT_PATH="/home/users0/jagfelga/slu-work/NLG/RNNLG-Laptop/model_word_based_delex/1/"

mkdir -p $EXPT_PATH

# data preprocessing

#python $OPENNMT_PATH/preprocess.py -train_src $DATA_PATH/train_src.txt -train_tgt $DATA_PATH/train_tgt.txt -src_seq_length 47 -valid_src $DATA_PATH/dev_src.txt -valid_tgt $DATA_PATH/dev_tgt.txt -tgt_seq_length 67 -save_data $DATA_PATH
...
# test on test data
#python ${OPENNMT_PATH}/translate.py -model $EXPT_PATH/_acc_*_e13.pt -src $DATA_PATH/test_src.txt -output $EXPT_PATH/test_e13.txt -verbose -gpu 0 -max_length 75 -beam_size 10 >& $EXPT_PATH/test_e13.log &
