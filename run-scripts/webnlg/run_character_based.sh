...
DATA_PATH="../../datasets/webnlg/preprocessed/character_based/"
EXPT_PATH="../../experiments/webnlg/character_based/"
...
if [ "$PREPROCESS" = true ]
    ...
    python $OPENNMT_PATH/preprocess.py -train_src $DATA_PATH/train_src.txt -train_tgt $DATA_PATH/train_tgt.txt\
     -src_seq_length 4000 -valid_src $DATA_PATH/dev_src.txt -valid_tgt $DATA_PATH/dev_tgt.txt -tgt_seq_length 4000\
...
then
	...
	do
		...
		nohup python -u ${OPENNMT_PATH}/translate.py -model $EXPT_PATH/"$MODEL_NAME"_"$NUM_MODEL".pt -src $TEST_FILE_SRC\
		 -output $TEST_FILE_TGT_HYPOTHESES -gpu 0 -max_length 400 -symbol_representation char2char -n_best 1 -beam_size $BEAM\
