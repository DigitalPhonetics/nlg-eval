...
def build_save_text_dataset_in_shards(src_corpus, tgt_corpus, fields,
    ...
    ret_list = []
    src_symbol_representation, tgt_symbol_representation = opt.symbol_representation.split("2")
    src_iter = onmt.io.ShardedTextCorpusIterator(
        src_corpus, opt.src_seq_length_trunc,
        "src", opt.max_shard_size, src_symbol_representation)
    tgt_iter = onmt.io.ShardedTextCorpusIterator(
        tgt_corpus, opt.tgt_seq_length_trunc,
        "tgt", opt.max_shard_size,
        tgt_symbol_representation,
        revert=opt.revert_targets,
        assoc_iter=src_iter)
    ...
    while not src_iter.hit_end():
        ...
        print(" * saving %s data shard with %d examples to %s." % (corpus_type, len(dataset), pt_file))
    ...
    print (" * in total saved %d %s examples in %d shards" % (num_examples, corpus_type, index))
