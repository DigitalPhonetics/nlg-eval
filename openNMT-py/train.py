...
def train_model(model, fields, optim, data_type, model_opt):
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        if valid_stats.ppl() < last_valid_ppl:
            not_improved = 0
            if opt.overwrite_checkpoints or (epoch >= opt.start_checkpoint_at):
                trainer.drop_checkpoint(model_opt, epoch, fields, valid_stats, overwrite_checkpoints=opt.overwrite_checkpoints)
        else:
            not_improved += 1

        #if not_improved > opt.stop_after_no_improvement_for:
        #    print('Stop training because ppl did not improve on validation set for %d epochs'
        #          %opt.stop_after_no_improvement_for)
...
def build_optim(model, checkpoint):
    if opt.train_from:
        ...
        optim = onmt.Optim(
            ...
            weight_decay=opt.l2_regularization_weight)
