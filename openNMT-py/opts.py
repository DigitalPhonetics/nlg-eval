...
def preprocess_opts(parser):
    ...
    group.add_argument('-symbol_representation', choices=['word2word', 'char2char'], default='word2word',
                       help="Representation of the input and output text")
    group.add_argument('-revert_targets', action='store_true',
                       help="Revert targets to train backward model")
...
def train_opts(parser):
    ...
    group.add_argument('-overwrite_checkpoints', action='store_true',
                       help="""Overwrite models during training such that only model with lowest ppl remains""")
    # GPU
    ...
    group.add_argument('-dropout', type=float, default=0.3,
                       help="Probability of RNN output to be zeroed,"
                            "only applied in non-terminal layers of stacked RNNs")
    group.add_argument('-l2_regularization_weight', type=float, default = 0.0,
                       help="Weight of L2 regularization applied in optimizer")
...
def translate_opts(parser):
    ...
    group.add_argument('-symbol_representation', choices=['word2word', 'char2char', 'word2char'], default='word2word',
                       help="Representation of the input and output text")
    group.add_argument('-revert_targets', action='store_true', help="Revert targets to train backward model")
    ...
    parser.add_argument('-num_variations', type=str, default="",
                        help='Number of hypotheses that are generated for each input (random selection from'
                             '"n_best" beam search output provided as "," separated list of integers'
                             '(defaults to n_best)')
    parser.add_argument('-stochastic', action='store_true', default=False,
                        help='Random selection of the "num_variations"/"n_best" hypotheses among the "n_best" ones')
    group.add_argument('-report_individual_scores', action="store_true",
                       help="""Output the score of the prediction (and the target, if targets are provided)
                       separated by tab in each line after the prediction""")
