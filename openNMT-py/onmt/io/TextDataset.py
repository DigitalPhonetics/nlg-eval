...
class TextDataset(ONMTDatasetBase):
    ...
    @staticmethod
    def make_text_examples_nfeats_tpl(path, truncate, side, symbol_representation, revert=False):
        """
        Args:
            path (str): location of a src or tgt file.
            truncate (int): maximum sequence length (0 for unlimited).
            side (str): "src" or "tgt".
            symbol_representation: linguistic level text representation (word or char)
        ...
        examples_nfeats_iter = \
            TextDataset.read_text_file(path, truncate, side, symbol_representation, revert)
    ...
    @staticmethod
    def read_text_file(path, truncate, side, symbol_representation, revert=False):
        """
        Args:
            path (str): location of a src or tgt file.
            truncate (int): maximum sequence length (0 for unlimited).
            side (str): "src" or "tgt".
            revert (bool): whether to reverse the sequence or not
            symbol_representation: linguistic level of the text representation (word or char), determines splitting
        ...
        with codecs.open(path, "r", "utf-8") as corpus_file:
            for i, line in enumerate(corpus_file):
                if symbol_representation == "char":
                    line = list(line.strip())
                elif symbol_representation == "word":
                    line = line.split()
                if truncate:
                    line = line[:truncate]
...
class ShardedTextCorpusIterator(object):
    ...
    def __init__(self, corpus_path, line_truncate, side, shard_size,
                 symbol_representation, revert=False, assoc_iter=None):
        """
        Args:
            ...
            symbol_representation: linguistic level of input and output representation (word or char)
            revert: revert the input sequence (words or characters, also features if present)
        ...
        self.line_index = -1
        self.eof = False
        self.symbol_representation = symbol_representation
        self.revert = revert
