...
def process_e2e_mr(s):
    ...
    for idx, item in enumerate(items):
        ...
        # normalize upper/lowercasing of the attribute name
        if key not in k2v:
            key = key.lower()
        ...
        if key not in k2v:
            print "found unknown key %s with value %s" % (key, value)
...
def _get_loc(area_val, near_val, snt):
    ...
    if area_val:
        s = "%s is located in the %s area" % (beginning, area_val)		# SB: because of the inverted sentence beginning, we have to make sure that EVERY instance of "located in" is preceded by "is".
        if near_val:
            s += ", near %s." % near_val
        ...
        s = "%s is located near %s." % (beginning, near_val)			# SB: because of the inverted sentence beginning, we have to make sure that EVERY instance of "located in" is preceded by "is".
...
def _get_loc2(area_val, near_val):
    ...
    """
    if area_val:
        s = " is located in the %s area" % area_val
        if near_val:
            s += ", near %s." % near_val
        ...
        s = " is located near %s." % near_val					# SB: because of the inverted sentence beginning, we have to make sure that EVERY instance of "located in" is preceded by "is".
...
def make_prediction(xd):
    ...
    if friendly is True:
        snt = "The family-friendly %s %s" % (restaurant_type, name)			# SB: switch name and restaurant type for the beginning of the sentence.
    else:
        snt = "The %s %s" % (restaurant_type, name)					# SB: switch name and restaurant type for the beginning of the sentence.
    ...
    if food_type or price_range:
        ...
        snt += " serves %s" % food							# SB: since the preceding token is NAME-X, take out "which".
        price_range = _get_price_str(price_range)
        snt += price_range
        # ends with a dot

        if near or area:								# SB: in the following, switch the if/elif conditions on near/area and rating. 
            snt = snt[:-1]  # strip the trailing dot
            snt = _get_loc(area, near, snt)
            # ends with a dot
            if rating:
                snt = _get_rating(rating, snt)
                snt += "."								# SB: add full stop
...
def run(fname, mode='dev'):
    ...
    # Saving predictions
    predictions_fname = "%s.predicted_2" % fname
    with open(predictions_fname, "w") as out:
        logger.info("Saving predictions to --> %s" % predictions_fname)

        if mode == 'test':
            # a CSV file with paired MR and predictions (like original dataset)
            out.write("mr,ref\n")
