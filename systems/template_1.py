...
def make_prediction(xd):
    ...
    if rating:
        snt = _get_rating(rating, snt)			# SB: the following two portions for rating and near/area were originally the other way around. By switching them, we fix their order without having to comment them out!
...
def run(fname, mode='dev'):
    ...
    # Saving predictions
    predictions_fname = "%s.predicted_1" % fname
